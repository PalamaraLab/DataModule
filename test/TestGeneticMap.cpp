...
TEST_CASE("GeneticMap: test exceptions", "[GeneticMap]") {
  ...
  CHECK_THROWS_WITH(GeneticMap(nonexistentFile), StartsWith("Error: genetic map file"));
  CHECK_THROWS_WITH(GeneticMap(nonexistentFile), EndsWith("does not exist\n"));
  ...
  CHECK_THROWS_WITH(GeneticMap(spaceSepFile),
                    ContainsSubstring("should contain at least one data row with at least 3 tab-separated columns"));
  ...
  CHECK_THROWS_WITH(GeneticMap(inconsistentNumCols),
                    ContainsSubstring("line 2 contains 3 columns, but the first data row contains 4"));
  ...
  CHECK_THROWS_WITH(GeneticMap(badCol1), ContainsSubstring("line 3 should contain an unsigned integer"));
  CHECK_THROWS_WITH(GeneticMap(badCol3), ContainsSubstring("line 4 should contain an unsigned integer"));
...
TEST_CASE("GeneticMap: disambiguate from PLINK map", "[GeneticMap]") {
...

  {
    ...
    CHECK_THAT(buffer.str(), ContainsSubstring("Warning: genetic map file") && ContainsSubstring("indices 0 and 1"));
    std::cout.rdbuf(old);
  }

  CHECK_THROWS_WITH(GeneticMap(plinkMap4Col), ContainsSubstring("should contain at least one data row with at least 3"));
}
...
TEST_CASE("GeneticMap: warn on Mbp-cM range", "[GeneticMap]") {
  ...
  CHECK_THAT(buffer.str(), ContainsSubstring("Warning: 20.0% of entries"));
  std::cout.rdbuf(old);
}
